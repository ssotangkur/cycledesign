...
```
Delegate to the git-create-branch skill:
...
```
/git-create-branch 41 issue-processing-framework
/git-create-branch https://github.com/ssotangkur/cycledesign/issues/37
```
...
1. **Fetch issue via `gh` CLI**:
   ```bash
   gh issue view {issueNumber} --repo ssotangkur/cycledesign --json title
   ```
...
2. **Extract and convert title to kebab-case**:
   ...
   ```
   Lowercase, strip punctuation, replace spaces/underscores with `-`, collapse repeats, trim leading/trailing `-`.

3. **Use generated description** for branch name

## Authentication

Uses the `gh` CLI session for GitHub API access. In sandbox environments where `gh` is not authenticated, fall back to `GH_TOKEN`:
