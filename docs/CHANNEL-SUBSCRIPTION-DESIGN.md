...
1. Client calls: client.channel('chat')
   ↓
2. Client sends: { type: 'subscribe', channelType: 'chat' }
   ↓
3. Server receives subscription
   ↓
4. Server calls onSubscribe callback
   ↓
5. Client can immediately send channel messages
   ↓
6. Server queues messages until callback completes
   ↓
7. Messages flow bidirectionally
   ↓
8. Client disconnects or unsubscribes
   ↓
9. Server removes subscriber
   ↓
10. Cleanup function called
```

**Key Points:**
- Connection is established automatically on first `channel()` call
- Each `channel()` call creates an independent channel instance
- No subscribe-ack - server uses queuing for flow control
...
Client                              Server
  ...
  │                                   │
  │────── { type: 'subscribe',        │
  │         channelType: 'chat' } ───▶│  (no channelId)
  │                                   │
  │◀───── { type: 'subscribe-response',│
  │         channelType: 'chat',      │
  │         channelId: 'channel-1' } ─│  (server generates)
  │                                   │
...
class ProtocolClient {
  ...
  private channels = new Map<string, Channel<any>>();  // channelId → channel
  private pendingSubscriptions = new Map<string, (channelId: string) => void>();  // channelType → resolve
  private url: string;
  ...
  channel<T extends keyof ChannelTypes>(channelType: T): Channel<ChannelTypes[T]> {
    ...
    const channelId = `client-channel-${this.nextChannelId++}`;
    ...
    const channel = new ChannelImpl(channelId, channelType as string, this);
    this.channels.set(channelId, channel);

    this.pendingSubscriptions.set(channelType, (serverChannelId: string) => {
      channel.updateServerChannelId(serverChannelId);
      this.channels.set(serverChannelId, channel);
      this.channels.delete(channelId);
      this.pendingSubscriptions.delete(channelType);
    });
  ...
  private connect(): void {
    ...
    this.ws.onmessage = (event) => {
      ...
      if (message.type === 'subscribe-response') {
        ...
        const resolve = this.pendingSubscriptions.get(channelType);
        if (resolve) {
          resolve(channelId);
        }
 ...
 * 
 * Protocol flow:
 * 1. Client sends 'subscribe' with channelType only (no channelId)
 * 2. Server generates channelId and responds with 'subscribe-response'
 * 3. Client uses assigned channelId for all subsequent messages
 */
type ControlMessage =
  /** 
   ...
   */
  | { type: 'subscribe'; channelType: string }

  /** 
   * Server responds with generated channelId
   * Client uses this channelId for all subsequent messages
   */
  | { type: 'subscribe-response'; channelType: string; channelId: string }
